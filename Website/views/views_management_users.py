...
def listUsersView(request):
    users = UserMetadata.objects.filter(user__is_active=True, fire_stations_id=request.session.get('fire_station_id'))
        ...
        if form.is_valid():
            ...
            #¿Los campos selects tiene valores válidos?
                ...
                try:
                    ...
                    UserMetadata.objects.create(number=request.POST['numberField'], fire_stations_id=request.session.get('fire_station_id'), profiles_id=request.POST['profile'], user_id=u.id)
                    ...
                    messages.add_message(request, messages.WARNING, f"Ocurrió un error inesperado. Por favor, vuelva a intentarlo.")
                    return HttpResponseRedirect(reverse('website-ruta_list_users'))
...
def updateUserView(request, user_id):
    ...
    #Ya que por algún motivo mis funciones js para manejar listas JSON no funcionaron en esta vista, obtendré el listado de perfiles de esta manera, ignorando el perfil seleccionado
    ...
    initial_data = {
        'rutField': userMeta.user.username,
    ...
    form = UpdateUserForm(request.POST or None, initial=initial_data)

    if request.method == 'POST':
        if form.is_valid():
            #¿El rut existe?
            if (User.objects.filter(username=request.POST['rutField']).exclude(id=user_id).exists()):
                messages.add_message(request, messages.WARNING, f"El rut ya está en uso.")
                return HttpResponseRedirect(reverse('website-ruta_update_user', args=[user_id]))
            ...
            #¿El correo existe?
            elif(User.objects.filter(email=request.POST['emailField']).exclude(id=user_id).exists()):
                messages.add_message(request, messages.WARNING, f"El correo ya está en uso.")
                return HttpResponseRedirect(reverse('website-ruta_update_user', args=[user_id]))
                ...
                userMeta.number = request.POST.get('numberField')
...
def deleteUserView(request, user_id):
    ...
    return render(request, 'Website/management_users/delete_user.html', {'user_active':user})


#Vista - Aprobar nuevos usuarios
@login_required()
def listApproveUsersView(request):
    users = UserMetadata.objects.filter(user__is_active=False, fire_stations_id=request.session.get('fire_station_id'))
    total_users = users.count()
...
@login_required()
#Eliminar usuario
def deleteUser(request, user_id, template):
    try:
        user = User.objects.filter(id=user_id).get()
        user.delete()
