...
class FireTruck(models.Model):
    ...
    #Año (opcional)
    year = models.CharField(verbose_name="Año", max_length=4, blank=True, null=True, help_text="(Opcional) Ingrese el año de fabricación del vehículo")
    #Tipo de unidad
    ...
    #Imagen (opcional)
    image = models.ImageField(verbose_name="Imagen", upload_to="firetrucks", default="default-firetruck.png", blank=True, null=True, help_text="(Opcional) Subir imagen del vehículo")
    #Fecha de creación
...
class Compartment(models.Model):
    ...
    def __str__(self):
        return f"{self.fire_trucks.name} - {self.name}"
...
class Inventory(models.Model):
    #Código de barras
    sku = models.BigIntegerField(verbose_name="Código de barras", unique=True, blank=True, null=True)
    #Categoría
    categories = models.ForeignKey(Category, on_delete=models.DO_NOTHING, verbose_name="Categoría", help_text="Seleccionar categoría")
    #Ubicación (gabeta)
    compartments = models.ForeignKey(Compartment, on_delete=models.CASCADE, verbose_name="Gabeta/compartimento", help_text="Seleccionar en qué gabeta está ubicado")
    #Estado
    status = models.ForeignKey(Status, on_delete=models.DO_NOTHING, verbose_name="Estado", help_text="Definir estado del insumo")
    #Fecha de expiración (opcional)
    expire_at = models.DateField(verbose_name="Fecha de Vencimiento", blank=True, null=True, default=None, help_text="(Opcional) Indicar fecha de expiración si corresponde")
    ...
    maintenance_hour = models.IntegerField(verbose_name="Horas de uso para mantenimiento", blank=True, null=True, default=None, help_text="(Opcional) Indicar cada cuántas horas de uso se debe hacer mantenimiento")
    #Horas de uso (opcional)
    usage_hours = models.IntegerField(verbose_name="Horas de uso", blank=True, null=True, default=None, help_text="(Opcional) Horas de uso reseteables")
    #Total horas de uso (opcional)
    total_usage_hours = models.IntegerField(verbose_name="Horas de uso totales", blank=True, null=True, default=None, help_text="(Opcional) Horas de uso totales. Por defecto es cero")
    #Insumo contenedor (opcional)
    container = models.ForeignKey('self', on_delete=models.CASCADE, verbose_name="Insumo contenedor", blank=True, null=True, default=None, help_text="(Opcional) Si el insumo está contenido dentro de otro insumo, indicarlo aquí")
    #Programa de mantenimiento (opcional)
    maintenance_programs = models.ForeignKey(MaintenanceProgram, on_delete=models.DO_NOTHING, verbose_name="Programa de mantenimiento" ,blank=True, null=True, default=None, help_text="(Opcional) Indicar programa de mantención si corresponde")
    #Fecha de última mantención
    last_maintenance = models.DateField(verbose_name="Última mantención", blank=True, null=True, default=None)
